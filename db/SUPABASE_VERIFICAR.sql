-- ############################################################################
-- #   MS BARBERÍA — VERIFICACIÓN PREVIA  (correr ANTES de instalar)          #
-- ############################################################################
--
...
--   PROBLEMA → resolvelo antes de correr SUPABASE_INSTALAR_TODO.sql.
-- ============================================================================
...
chk_es_barbero as (
  select 'función es_barbero()',
         ...
         'Todas las políticas RLS nuevas dependen de esta función. Si falta, la crea la PARTE A del instalador.'
),
chk_perfiles as (
...
chk_dup_config as (
  select 'fechas duplicadas en config_jornada',
         case when count(*) = 0 then 'OK' else 'ATENCION' end,
         format('%s fecha(s) repetidas; el instalador se queda con la más reciente.', count(*))
...
chk_tabla_experiencia as (
  select 'tabla duplicada `experiencia`',
         ...
         'Convive con `experiencia_items`, que es la que usa el código. La limpieza opcional está en migrations/009_limpieza_opcional.sql.'
),
...
chk_politica_peligrosa as (
  select 'política pública de inserción en dias_libres',
         ...
         'Si existe, cualquier visitante anónimo puede insertar días libres y cerrar la barbería. La elimina la PARTE B del instalador.'
),
-- ---------- Volumen ----------
chk_volumen as (
